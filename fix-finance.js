const fs = require("fs");
const path = "c:/MFO-CRM/apps/web/app/(dashboard)/platform/finance/page.tsx";
let code = fs.readFileSync(path, 'utf8');

const splitPointStart = '<TabPanels className="mt-0 flex-1 flex flex-col min-h-0 relative overflow-hidden h-full shadow-inner">';

const top = code.substring(0, code.indexOf(splitPointStart) + splitPointStart.length);
// Extract the very bottom which just has TabPanels closing tag and the rest.
// But since the file has duplicate FinanceHubPage at the end, let's just use the known correct ending:
const bottom = `
          </TabPanels>
        </main>
        </TabGroup>
...
fs.writeFileSync(path, top + middle + bottom);
console.log("Finance Page completely repaired and restored.");
