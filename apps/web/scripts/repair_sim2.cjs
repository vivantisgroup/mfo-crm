...
const startTarget = '            setNominalRate(data.nominal);';
const endTarget = '            onshoreTax = jRules.capGainsOnshore / 100;';

const startIdx = txt.indexOf(startTarget);
const endIdx = txt.indexOf(endTarget);

if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
    const toReplace = txt.substring(startIdx, endIdx + endTarget.length);
    console.log("Found substring to replace. Length: ", toReplace.length);
...
const runPath = useCallback((isMonteCarlo = false) => {
    ...
    txt = txt.substring(0, startIdx) + fixStr + txt.substring(endIdx + endTarget.length);
