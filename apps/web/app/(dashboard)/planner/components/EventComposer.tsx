...
import { Calendar, Clock, Contact, X, Video, FileText, Network } from 'lucide-react';
...
export default function EventComposer({ isOpen, onClose, eventToEdit, selectedDate, provider, onSaved }: EventComposerProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
         if (!tenant?.id) return;
         const snap = await getDocs(query(collection(db, `tenants/${tenant.id}/contacts`), orderBy('name')));
  ...
  useEffect(() => {
    if (isOpen) {
      if (eventToEdit) {
        ...
        // Extract family tag from description if exists
        let desc = eventToEdit.description || '';
        const match = desc.match(/\[CRM_FAMILY:(.*?)\]/);
        if (match) {
           setFamilyName(match[1].trim());
           desc = desc.replace(match[0], '').trim();
        } else if (eventToEdit.familyName) {
           setFamilyName(eventToEdit.familyName);
        } else {
           setFamilyName('');
        }
        setDesc(desc);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-sm transition-all duration-300">
      <div className="w-full max-w-xl bg-white max-h-[90vh] rounded-3xl shadow-2xl flex flex-col animate-in zoom-in-95 fade-in duration-200 overflow-hidden border border-slate-200/60">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6">
          ...
          <div>
             <label className="text-sm font-semibold text-slate-700 flex items-center gap-2 mb-2">
              <Network size={16} /> Associated Family / Group
             </label>
             <input 
               className="w-full text-sm rounded-lg border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
               value={familyName}
