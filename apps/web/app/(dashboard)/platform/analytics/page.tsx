...
export default function AnalyticsPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const mrr = activeSubs.reduce(
      (sum, s) => sum + (PLAN_PRICES[s.planId?.toLowerCase()] ?? 1000),
      0,
    );
    const arr = mrr * 12;
    ...
    const monthlyOpex = activeExpenses.reduce(
    ...
    const churnRate =
      totalTenants > 0 ? (churnedCount / totalTenants) * 100 : 0;
    ...
    // CRM pipeline
    const openOpportunities = orgs.filter(
      (o) => !["closed_won", "closed_lost"].includes(o.stage),
    ).length;
    const totalPipelineAum = orgs
      .filter((o) => !["closed_lost"].includes(o.stage))
      .reduce((s, o) => s + o.estAumUsd, 0);

    // Expense breakdown by category
    const expByCategory = Object.keys(EXPENSE_CATEGORIES).reduce(
      (acc, k) => {
        acc[k] = activeExpenses
          ...
          <div className="animate-fade-in">
            ...
            >
              <KpiCard
                ...
                color="#6366f1"
                trend={metrics.mrr > 0 ? "+11.2% MoM" : undefined}
              />
              <KpiCard
                label="ARR"
              ...
              <KpiCard
                ...
                sub={fmtPct(metrics.grossMargin) + " gross margin"}
                color="#22c55e"
              />
              <KpiCard
                label="Monthly OPEX"
                value={fmt(metrics.monthlyOpex)}
                sub="Active platform expenses"
                color="#ef4444"
              />
            </div>
            <div
            ...
            >
              <KpiCard
                label="LTV / CAC"
                value={fmtX(metrics.ltvCacRatio)}
                sub={`Target: >3x ${metrics.ltvCacRatio >= 3 ? "✅" : "⚠️"}`}
                color="#a78bfa"
              />
              <KpiCard
                label="Payback Period"
                value={`${Math.round(metrics.payback)}mo`}
                sub={`Target: <12mo ${metrics.payback <= 12 ? "✅" : "⚠️"}`}
                color="#f59e0b"
              />
              <KpiCard
                label="Active Tenants"
                value={metrics.activeCount}
                sub={`${metrics.trials} in trial · ${metrics.churnedCount} churned`}
                color="#22c55e"
              />
              <KpiCard
                label="Churn Rate"
                value={fmtPct(metrics.churnRate)}
                sub="Suspended + cancelled"
                color={metrics.churnRate > 10 ? "#ef4444" : "#22c55e"}
              />
            </div>
            ...
            >
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Monthly snapshot with gross margin"
                />
                <div
                  style={{
                    display: "flex",
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${Math.min(100, (metrics.monthlyOpex / metrics.mrr) * 100)}%`,
                          background: "#ef4444",
                          borderRadius: 4,
                        }}
                      />
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${Math.max(0, metrics.grossMargin)}%`,
                          background: "#22c55e",
                          borderRadius: 4,
                        }}
                      />
                    ...
                    >
                      {fmt(metrics.grossProfit)} ({fmtPct(metrics.grossMargin)})
                    </div>
                  </div>
                </div>
                <div
                      ...
                      return (
                        <div
                          ...
                          style={{
                            flex: 1,
                            height: h,
                            background: isLast ? "#6366f1" : "#6366f133",
                            borderRadius: 3,
                            transition: "height 0.4s",
                            minHeight: 3,
                          }}
                          title={`Month ${i + 1}: ${fmt(v)}`}
                        />
                      );
                    })}
                  </div>
              ...
              >
                <SectionHeader
                  title="CRM Pipeline"
                  subtitle="Open opportunities and AUM in play"
                />
                <div
                  style={{
                    display: "grid",
                ...
                >
                  {[
                    ...
                    },
                    {
                      label: "Pipeline AUM",
                      value:
                        metrics.totalPipelineAum >= 1e9
                          ? `$${(metrics.totalPipelineAum / 1e9).toFixed(1)}B`
                          : `$${(metrics.totalPipelineAum / 1e6).toFixed(0)}M`,
                      color: "#a78bfa",
                    },
                  ].map((m) => (
                    <div
                ...
                <div>
                  {[
                    ...
                    const pct =
                      orgs.length > 0 ? (count / orgs.length) * 100 : 0;
            ...
            <div
              style={{
                background: "linear-gradient(135deg, #6366f108, #22d3ee08)",
            ...
            >
              <SectionHeader
                ...
                subtitle="Key business health indicators with guidance"
              />
              <div
                style={{
                  display: "grid",
              ...
              >
                {[
                  ...
                  },
                  {
                    ...
                    value: fmtPct(
                      metrics.mrr > 0
                        ? (metrics.monthlyOpex / metrics.mrr) * 100
                        : 100,
                    ),
                    status:
                      metrics.monthlyOpex / Math.max(metrics.mrr, 1) <= 0.4
                        ? "good"
                        : metrics.monthlyOpex / Math.max(metrics.mrr, 1) <= 0.6
                          ? "warn"
                          : "bad",
                    target: "Target: ≤40% of MRR",
                  },
                ].map((h) => {
                  const colors = {
                    good: "#22c55e",
        ...
        {tab === "revenue" && (
          <div className="animate-fade-in">
            ...
            >
              ...
              <KpiCard
                label="Avg MRR"
                value={fmt(metrics.avgMrr)}
                sub="Per active tenant"
                color="#a78bfa"
              />
              <KpiCard
                label="NRR"
                ...
                color="#22c55e"
                trend={`${metrics.nrr >= 100 ? "+" : ""}${metrics.nrr - 100}% expansion`}
              />
              <KpiCard
                label="Tenants"
                value={metrics.activeCount}
                sub={`${metrics.trials} trial · ${metrics.churnedCount} churned`}
                color="#f59e0b"
              />
              <KpiCard
                label="Trials"
                value={metrics.trials}
                sub="Free trial tenants"
                color="#22d3ee"
              />
            </div>
            <div
            ...
            >
              <SectionHeader
                ...
                subtitle="MRR breakdown across service tiers"
              />
              {[
                ...
                const planSubs = subs.filter(
                  (s) =>
                    s.planId?.toLowerCase().includes(plan.toLowerCase()) &&
                    s.status === "active",
                );
                ...
                return (
                  <ProgressBar
                    key={plan}
                    label={`${plan} (${planSubs.length} tenants)`}
                    value={revenue}
                    max={metrics.mrr}
                    color={color}
            ...
            >
              <SectionHeader
                title="Monthly MRR Trend (simulated)"
                subtitle="12-month MRR growth projection"
              />
              <div
                style={{
                  display: "flex",
              ...
              >
                {metrics.mrrHistory.map((v, i) => {
                  ...
                  const mo = new Date(2026, 2 - (11 - i), 1).toLocaleString(
                    "en-US",
                    { month: "short" },
                  );
                  return (
                    ...
                    >
                      ...
                      <div
                        style={{
                          width: "100%",
                          height: h,
                          background: i === 11 ? "#6366f1" : "#6366f144",
          ...
          <div className="animate-fade-in">
            ...
            >
              <KpiCard
                label="Monthly OPEX"
                value={fmt(metrics.monthlyOpex)}
                sub="All active recurring expenses"
                color="#ef4444"
              />
              <KpiCard
                label="Annual OPEX"
                value={fmt(metrics.annualOpex)}
              ...
              <KpiCard
                ...
                sub={
                  metrics.mrr > 0 && metrics.monthlyOpex / metrics.mrr <= 0.4
                    ? "✅ Healthy ratio"
                    : "⚠️ Above target"
                }
                color={
            ...
            >
              ...
              >
                ...
                />
                {Object.entries(metrics.expByCategory)
                  .filter(([, v]) => v > 0)
                  .sort(([, a], [, b]) => b - a)
                  .map(([k, v]) => {
                    const { label, icon, color } =
                      EXPENSE_CATEGORIES[k as keyof typeof EXPENSE_CATEGORIES];
                    return (
                      <ProgressBar
                        key={k}
                        label={`${icon} ${label}`}
                        value={v}
                        max={metrics.monthlyOpex}
                        color={color}
                        sublabel={`${metrics.monthlyOpex > 0 ? Math.round((v / metrics.monthlyOpex) * 100) : 0}% · ${fmt(v * 12)}/yr`}
                      />
                    );
                  })}
              </div>
              ...
              >
                <SectionHeader
                  title="Top Expenses"
                  subtitle="By monthly cost (active only)"
                />
                {expenses
                  .filter((e) => e.active)
          ...
          <div className="animate-fade-in">
            ...
            >
                ...
                {(
                  ...
                  return (
                    <ProgressBar
                      ...
                      max={subs.length}
                      color={colors[status]}
                      sublabel={`${subs.length > 0 ? Math.round((count / subs.length) * 100) : 0}% of all tenants · ${count} tenant${count !== 1 ? "s" : ""}`}
                    />
                  );
                })}
              </div>
              ...
              >
                <SectionHeader
                  title="Tenant List"
                  subtitle="All subscriptions with status"
                />
                <div style={{ overflowY: "auto", maxHeight: 320 }}>
                  {subs.length === 0 ? (
                    ...
                    >
                      No tenant data found. Go to Tenant Management to add
                      tenants.
                    </div>
                  ) : (
                    subs.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            >
                              {s.planId} · {s.licensedSeats} seats
                            </div>
                          </div>
                          <span
                            style={{
        ...
        {tab === "roi" && (
          <div className="animate-fade-in">
            <div
              style={{
                background: "linear-gradient(135deg, #6366f108, #22d3ee08)",
                border: "1px solid var(--brand-500)44",
                borderRadius: "var(--radius-xl)",
                padding: "28px 32px",
                marginBottom: 28,
              }}
            >
              <SectionHeader
                ...
                subtitle="Core business health metrics to evaluate platform performance and growth sustainability"
              />
              <div
                style={{
                  display: "grid",
                ...
                {[
                  {
                    ...
                    color: "#6366f1",
                    detail:
                      "Avg MRR × 24 months. Increases as we grow avg contract value and reduce churn.",
                  },
                  {
                    label: "CAC",
                    ...
                    color: "#f59e0b",
                    detail:
                      "Sales + marketing cost per new customer. Target: reduce via inbound and referrals.",
                  },
                  {
                    label: "LTV / CAC Ratio",
                    value: fmtX(metrics.ltvCacRatio),
                    sub:
                      metrics.ltvCacRatio >= 3
                        ? "✅ Healthy (>3x)"
                        : "⚠️ Needs work (<3x)",
                    color: metrics.ltvCacRatio >= 3 ? "#22c55e" : "#ef4444",
                    detail:
                      "Industry benchmark: SaaS businesses target >3x for sustainable growth.",
                  },
                  {
                    label: "Payback Period",
                    value: `${Math.round(metrics.payback)} months`,
                    sub:
                      metrics.payback <= 12 ? "✅ <12 months" : "⚠️ >12 months",
                    color: metrics.payback <= 12 ? "#22c55e" : "#ef4444",
                    detail:
                      "Months to recover CAC from MRR. Industry target: <12 months.",
                  },
                ].map((m) => (
                  <div
                    key={m.label}
            ...
            >
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Monthly P&L breakdown"
                />
                {[
                  {
                    label: "Gross Revenue (MRR)",
                    value: metrics.mrr,
                    color: "#22c55e",
                    sign: "+",
                  },
                  {
                    ...
                    value: -(metrics.expByCategory["infrastructure"] || 0),
                    color: "#ef4444",
                    sign: "",
                  },
                  {
                    label: "Software / SaaS",
                    value: -(metrics.expByCategory["software"] || 0),
                    color: "#ef4444",
                    sign: "",
                  },
                  {
                    label: "Personnel",
                    value: -(metrics.expByCategory["personnel"] || 0),
                    color: "#ef4444",
                    sign: "",
                  },
                  {
                    label: "Marketing",
                    value: -(metrics.expByCategory["marketing"] || 0),
                    color: "#ef4444",
                    sign: "",
                  },
                  {
                    label: "Legal & Other",
                    value: -(
                      (metrics.expByCategory["legal"] || 0) +
                      (metrics.expByCategory["office"] || 0) +
                      (metrics.expByCategory["other"] || 0)
                    ),
                    color: "#ef4444",
                    sign: "",
                  },
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontSize: 14,
                          fontWeight: 800,
                          color: row.value >= 0 ? "#22c55e" : "#ef4444",
                        }}
                      >
                        {row.value >= 0 ? "+" : ""}
                        {fmt(row.value)}
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 18,
                      color: metrics.grossProfit >= 0 ? "#22c55e" : "#ef4444",
                    }}
                  >
                    {fmt(metrics.grossProfit)}/mo
                  </span>
                </div>
              </div>
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Where we need to be in 12 months"
                />
                {[
                  {
                    metric: "MRR Target",
                    current: metrics.mrr,
                    target: metrics.mrr * 1.5,
                    color: "#6366f1",
                    unit: "",
                  },
                  {
                    metric: "Active Tenants",
                    current: metrics.activeCount,
                    target: Math.ceil(metrics.activeCount * 1.5),
                    color: "#22c55e",
                    unit: "",
                  },
                  {
                  ...
                  return (
                    <div key={g.metric} style={{ marginBottom: 18 }}>
                      ...
                      >
                        {Math.round(pct)}% of 12-month target
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="grid grid-cols-3 gap-6 mt-6">
              <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
                ...
                <div className="text-center text-slate-500">
                  ...
                  <button className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-2 px-4 rounded-md text-xs mt-4">
                    View Templates
                  </button>
                </div>
              </div>
