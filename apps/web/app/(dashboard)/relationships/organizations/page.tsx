...
function CreateOrgDrawer({ tenantId, onClose }: { tenantId: string; onClose: () => void }) {
 ...
 <div className="w-[480px] bg-white border-l border-slate-200 flex flex-col h-screen shadow-2xl relative">
 ...
 <input className="input" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Acme Holdings LLC" />
 </div>
 ...
 {TYPE_ICONS[t]} {t.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
 </option>
 ))}
 </select>
 </div>
 ...
 <input className="input" value={form.jurisdiction} onChange={e => setForm(p => ({ ...p, jurisdiction: e.target.value }))} placeholder="e.g. Delaware, Cayman Islands" />
 </div>
...
export default function OrganizationsPage() {
 ...
 <p className="mb-6 max-w-sm" style={{ color: 'var(--text-secondary)' }}>Organizations act as structural nodes mapping companies, trusts, and banks seamlessly into your CRM.</p>
